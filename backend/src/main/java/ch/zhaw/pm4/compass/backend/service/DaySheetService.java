...
public class DaySheetService {
	...
	public GetDaySheetDto getDaySheetById(Long id) throws DayNotFoundException {
		return convertDayToGetDayDto(
				daySheetRepository.getDaySheetById(id).orElseThrow(() -> new DayNotFoundException(id)));
	}

	public GetDaySheetDto getDaySheetByDate(Date date) throws DayNotFoundException {
		return convertDayToGetDayDto(
				daySheetRepository.getDaySheetByDate(date).orElseThrow(() -> new DayNotFoundException(date)));
	}

	public GetDaySheetDto updateDay(UpdateDaySheetDto updateDay) throws DayNotFoundException {
		DaySheet daySheet = daySheetRepository.getDaySheetById(updateDay.getId())
				.orElseThrow(() -> new DayNotFoundException(updateDay.getId()));
	...
	private GetDaySheetDto convertDayToGetDayDto(DaySheet daySheet) {
		return new GetDaySheetDto(daySheet.getId(), daySheet.getDay_report(), daySheet.getDate(),
				daySheet.getConfirmed(), daySheet.getTimestamps());
	}
}
