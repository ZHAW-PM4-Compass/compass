...
/**
 * Service class for managing incidents linked to day sheets. This includes
 * creating, updating, deleting incidents, and retrieving incident details.
 *
...
public class IncidentService {
	...
	/**
	 * Creates an incident and assigns it to an existing day sheet based on the
	 * provided date, or creates a new day sheet if one does not exist.
	 *
	 * @param createIncident The incident DTO to create.
	 * @return The created incident DTO.
	 * @throws DaySheetNotFoundException If the day sheet could not be found or
	 *                                   created.
	 */
	...
	public List<IncidentDto> getAll() {
		...
		return incidentRepository.findAll().stream().map(incident -> {
	...
	public IncidentDto convertEntityToDto(Incident entity, UserDto userDto) {
		return IncidentDto.builder().id(entity.getId()).title(entity.getTitle()).description(entity.getDescription())
				.date(entity.getDaySheet().getDate()).user(userDto).build();
	}
}
