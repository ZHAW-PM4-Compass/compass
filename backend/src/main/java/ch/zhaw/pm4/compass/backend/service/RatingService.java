...
/**
 * Service class for managing ratings within the system. This service handles
 * the creation, retrieval, and validation of ratings for categories within day
 * sheets, adhering to specific business rules and data integrity requirements.
 *
...
public class RatingService {
	...
	/**
	 * Creates a new rating based on the provided DTO. It validates the rating
	 * against the category rules and associates it with the specified day sheet.
	 *
	 * @param createRating The rating DTO to create.
	 * @return The newly created rating DTO.
	 * @throws RatingIsNotValidException If the rating is not within the valid range
	 *                                   for the category.
	...
	/**
	 * Records multiple category ratings for a day sheet. Ensures that only one
	 * rating per category is recorded unless explicitly allowed and checks if the
	 * user is the owner of the day sheet.
	 *
	 * @param categoryDtoList      List of category DTOs with their respective
	 *                             ratings.
	 * @param daySheetId           ID of the day sheet where ratings are to be
	 *                             recorded.
	 * @param userId               ID of the user recording the ratings.
	 * @param isSocialWorkerRating Indicates if the ratings are being recorded by a
	 *                             social worker.
	 * @throws TooManyRatingsPerCategoryException If multiple ratings for a single
	 *                                            category are attempted to be
	 *                                            recorded.
	 * @throws RatingIsNotValidException          If a rating does not meet the
	 *                                            category's specified range.
	 * @throws CategoryNotFoundException          If any specified category does not
	 *                                            exist.
	 * @throws DaySheetNotFoundException          If the specified day sheet does
	 *                                            not exist.
	 * @throws UserNotOwnerOfDaySheetException    If the user is not the owner of
	 *                                            the day sheet and tries to record
	 *                                            ratings.
	 */
	 ...
	 * @return A list of extended rating DTOs containing the ratings for the
	 *         specified date.
	 */
	 ...
	 *
	 * @param daySheetId The ID of the day sheet for which ratings are to be
	 *                   retrieved.
	...
	/**
	 * Converts a {@link RatingDto} to a {@link Rating} entity. This method is
	 * crucial for persisting rating information obtained from the API into the
	 * database.
	 *
	 * @param dto The {@link RatingDto} object containing the details to be
	...
	/**
	 * Converts a {@link Rating} entity back to a {@link RatingDto}. This conversion
	 * includes assembling related data such as the category and day sheet details
	 * into the DTO for comprehensive data transfer.
	 *
	 * @param entity The {@link Rating} entity to be converted.
	 * @return A {@link RatingDto} containing the details from the entity and its
	 *         related data.
	 */
