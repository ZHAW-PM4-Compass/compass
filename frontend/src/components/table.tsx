...
const Item = ({ item, columns, actions }: Readonly<{
  ...
  return (
    <tr className="bg-white mt-2 border-t-4 border-slate-100">
      {columns && columns.map((column, index) => {
        return (
          <td key={index} className="py-4 px-6 text-left text-md">{item[column.title]}</td>
        )
      })}
      <td className="py-4 px-6 text-right text-md">
        {actions && actions.map((action, index) => {
          return (
            <button key={index} onClick={action.onClick} className={`rounded-md hover:bg-slate-100 text-sm mr-2 px-2.5 py-2 ${!action.label && "w-10"}`}>
